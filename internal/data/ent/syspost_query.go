...
type SysPostQuery struct {
	...
	withPostsUser   *SysUserQuery
	withSysUserPost *SysUserPostQuery
...
// QueryPostsUser chains the current query on the "posts_user" edge.
func (spq *SysPostQuery) QueryPostsUser() *SysUserQuery {
...
// QuerySysUserPost chains the current query on the "sys_user_post" edge.
func (spq *SysPostQuery) QuerySysUserPost() *SysUserPostQuery {
	query := (&SysUserPostClient{config: spq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := spq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (spq *SysPostQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (spq *SysPostQuery) FirstIDX(ctx context.Context) uint64 {
...
func (spq *SysPostQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (spq *SysPostQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (spq *SysPostQuery) IDs(ctx context.Context) (ids []uint64, err error) {
...
func (spq *SysPostQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := spq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (spq *SysPostQuery) Clone() *SysPostQuery {
	...
	return &SysPostQuery{
		...
		withPostsUser:   spq.withPostsUser.Clone(),
		withSysUserPost: spq.withSysUserPost.Clone(),
...
// WithPostsUser tells the query-builder to eager-load the nodes that are connected to
// the "posts_user" edge. The optional arguments are used to configure the query builder of the edge.
func (spq *SysPostQuery) WithPostsUser(opts ...func(*SysUserQuery)) *SysPostQuery {
	query := (&SysUserClient{config: spq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	spq.withPostsUser = query
	return spq
}

// WithSysUserPost tells the query-builder to eager-load the nodes that are connected to
// the "sys_user_post" edge. The optional arguments are used to configure the query builder of the edge.
func (spq *SysPostQuery) WithSysUserPost(opts ...func(*SysUserPostQuery)) *SysPostQuery {
	query := (&SysUserPostClient{config: spq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	spq.withSysUserPost = query
	return spq
}
...
func (spq *SysPostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*SysPost, error) {
	var (
		...
		_spec       = spq.querySpec()
		loadedTypes = [2]bool{
			spq.withPostsUser != nil,
			spq.withSysUserPost != nil,
		}
	)
	...
	if query := spq.withPostsUser; query != nil {
		if err := spq.loadPostsUser(ctx, query, nodes,
			func(n *SysPost) { n.Edges.PostsUser = []*SysUser{} },
			func(n *SysPost, e *SysUser) { n.Edges.PostsUser = append(n.Edges.PostsUser, e) }); err != nil {
			return nil, err
		}
	}
	if query := spq.withSysUserPost; query != nil {
		if err := spq.loadSysUserPost(ctx, query, nodes,
			func(n *SysPost) { n.Edges.SysUserPost = []*SysUserPost{} },
			func(n *SysPost, e *SysUserPost) { n.Edges.SysUserPost = append(n.Edges.SysUserPost, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (spq *SysPostQuery) loadPostsUser(ctx context.Context, query *SysUserQuery, nodes []*SysPost, init func(*SysPost), assign func(*SysPost, *SysUser)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uint64]*SysPost)
	nids := make(map[uint64]map[*SysPost]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(syspost.PostsUserTable)
		s.Join(joinT).On(s.C(sysuser.FieldID), joinT.C(syspost.PostsUserPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(syspost.PostsUserPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(syspost.PostsUserPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "posts_user" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (spq *SysPostQuery) loadSysUserPost(ctx context.Context, query *SysUserPostQuery, nodes []*SysPost, init func(*SysPost), assign func(*SysPost, *SysUserPost)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*SysPost)
	for i := range nodes {
	...
	query.Where(predicate.SysUserPost(func(s *sql.Selector) {
		s.Where(sql.InValues(syspost.SysUserPostColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "sys_post_id" returned %v for node %v`, fk, n)
		}
		assign(node, n)
	}
	return nil
...
func (spq *SysPostQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(syspost.Table, syspost.Columns, sqlgraph.NewFieldSpec(syspost.FieldID, field.TypeUint64))
	_spec.From = spq.sql
