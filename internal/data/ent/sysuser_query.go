...
// QueryUsersPost chains the current query on the "users_post" edge.
func (suq *SysUserQuery) QueryUsersPost() *SysPostQuery {
...
func (suq *SysUserQuery) QuerySysUserPost() *SysUserPostQuery {
	query := (&SysUserPostClient{config: suq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := suq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (suq *SysUserQuery) IDs(ctx context.Context) (ids []uint64, err error) {
...
func (suq *SysUserQuery) Clone() *SysUserQuery {
	...
	return &SysUserQuery{
		...
		withUsersPost:   suq.withUsersPost.Clone(),
		withSysUserPost: suq.withSysUserPost.Clone(),
...
// WithUsersPost tells the query-builder to eager-load the nodes that are connected to
// the "users_post" edge. The optional arguments are used to configure the query builder of the edge.
func (suq *SysUserQuery) WithUsersPost(opts ...func(*SysPostQuery)) *SysUserQuery {
	query := (&SysPostClient{config: suq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	suq.withUsersPost = query
	return suq
}
...
func (suq *SysUserQuery) WithSysUserPost(opts ...func(*SysUserPostQuery)) *SysUserQuery {
	query := (&SysUserPostClient{config: suq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	suq.withSysUserPost = query
	return suq
}
...
//		GroupBy(sysuser.FieldCreatedAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (suq *SysUserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*SysUser, error) {
	var (
		...
		_spec       = suq.querySpec()
		loadedTypes = [2]bool{
			suq.withUsersPost != nil,
			suq.withSysUserPost != nil,
		}
	)
	...
	if query := suq.withUsersPost; query != nil {
		if err := suq.loadUsersPost(ctx, query, nodes,
			func(n *SysUser) { n.Edges.UsersPost = []*SysPost{} },
			func(n *SysUser, e *SysPost) { n.Edges.UsersPost = append(n.Edges.UsersPost, e) }); err != nil {
			return nil, err
		}
	}
	if query := suq.withSysUserPost; query != nil {
		if err := suq.loadSysUserPost(ctx, query, nodes,
			func(n *SysUser) { n.Edges.SysUserPost = []*SysUserPost{} },
			func(n *SysUser, e *SysUserPost) { n.Edges.SysUserPost = append(n.Edges.SysUserPost, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (suq *SysUserQuery) loadUsersPost(ctx context.Context, query *SysPostQuery, nodes []*SysUser, init func(*SysUser), assign func(*SysUser, *SysPost)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uint64]*SysUser)
	nids := make(map[uint64]map[*SysUser]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(sysuser.UsersPostTable)
		s.Join(joinT).On(s.C(syspost.FieldID), joinT.C(sysuser.UsersPostPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(sysuser.UsersPostPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(sysuser.UsersPostPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*SysPost](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "users_post" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (suq *SysUserQuery) loadSysUserPost(ctx context.Context, query *SysUserPostQuery, nodes []*SysUser, init func(*SysUser), assign func(*SysUser, *SysUserPost)) error {
	...
	query.Where(predicate.SysUserPost(func(s *sql.Selector) {
		s.Where(sql.InValues(sysuser.SysUserPostColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "sys_user_id" returned %v for node %v`, fk, n)
		}
		assign(node, n)
	}
	return nil
...
func (suq *SysUserQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(sysuser.Table, sysuser.Columns, sqlgraph.NewFieldSpec(sysuser.FieldID, field.TypeUint64))
	_spec.From = suq.sql
